...
func (r *Registry) Build() {
	...
	for _, gf := range r.m {
		for _, ga := range gf.arch {
			ga.File.Comments = gf.kinds.cleanComments(ga.File.Comments)
		}
	}
}
...
// Print outputs the consolidated objects into their own file, and updates the corresponding os_arch files.
...
`, cmd, pkg.Name)
	...
	doInput := func(name string, file *ast.File) (err error) {
		out, err := newbufferedFile(name)
		if err != nil {
			return err
		}
		defer handleClose(&err, out)
	...
	do := func(name string, gf *gofile) (err error) {
		...
		// Print custom header for current OS.
		osname := strings.Split(name, "_")[1]
		header := strings.Replace(header, "__OS__", osname, 1)
